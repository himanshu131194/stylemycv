 ...
 (async function(){
   ...
   let resumeSamples = {
     ...
     for(let parentFolder in resumeSamples){
         fs.mkdirSync(`./${container}/${parentFolder}`);
         let parent1 = `./${container}/${parentFolder}`;
         let professionals = resumeSamples[parentFolder];
         for(let y in professionals){
             fs.mkdirSync(`${parent1}/${y}`);
             let parent2 = `${parent1}/${y}`;
             ...
             for(let z of resumeArrays){
                 ...
                 fs.writeFileSync(`${parent2}/${slugId}_thumb.${ext}`, resImageThumb);
             }
         }

     }
     // let response = await rp({
     //    uri: "https://memeful.com/api/all",
     //    json: true
     // });
         ...
         // });
         // let resImageThumb = await rp({
         //     uri: response[x]['thumbnail'],
         //     encoding: null
         // });
         //console.log(resImage);

        // fs.writeFileSync(`./meme/original/${slugId}.jpg`, resImage);
     //}
     // console.log(dataObj);
    //INSERT DATA IN DB
    // let resImage = await rp({
    //     uri: 'https://i.memeful.com/template_thumbnails/522acc5982a5f/ymRP0RL.jpg',
    //     encoding: null
    // });
    ...
    //   encoding: null
    // }, (err, res)=>{
    //      fs.writeFile("./memes/test.jpg", res.body, function(err) {
    //         if(err)
